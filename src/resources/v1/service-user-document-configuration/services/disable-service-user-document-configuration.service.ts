...
import deactivateServiceUserDocumentConfigurationHelperService from "../helpers/operations/deactivate-service-user-document-configuration.helper.service";

class DisableServiceUserDocumentConfigurationService {
  public async execute(
    ...
    try {
      ...
      await findServiceUserDocumentConfigurationStateHelperService.isAlreadyInactive(
        config,
        serviceUserDocConfigErrorsMessages,
      );

      const deactivated =
        await deactivateServiceUserDocumentConfigurationHelperService.execute(
          config,
          session,
          userId,
          DbTransactions,
      ...
      return serviceUserDocConfigPayload(
        ...
        serviceUserDocConfigResponse(deactivated),
        DbTransactions,
      );
    } catch (error) {
...
export default new DisableServiceUserDocumentConfigurationService();
