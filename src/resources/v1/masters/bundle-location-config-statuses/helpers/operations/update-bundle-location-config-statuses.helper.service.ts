...
class updateBundleLocationConfigStatusesHelperService {
  constructor() {}
  ...
  public async execute(
    ...
    try {
      ...
      if (changes.length === 0) {
        const data = bundleLocationConfigStatusesErrorResponse(existing);
        throwError(
          "no_change_detected",
      ...
      rethrowIfKnown(
        error,
        "Error while updating bundle location config status",
        errorMap,
      );
    }
  }
...
export default new updateBundleLocationConfigStatusesHelperService();
