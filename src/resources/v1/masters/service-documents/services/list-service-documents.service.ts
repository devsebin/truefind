...
import { serviceDocumentPayload, populateFields } from "../service-documents.helper";
import { serviceDocumentListResponse } from "../service-documents.response";
import { serviceDocumentErrorsMessages } from "../service-documents.messages";

class listServiceDocumentsService {
    ...
    async execute(
        ...
        try {
            ...
            const data = {
                ...
                from: 1 + offset,
                rows: serviceDocumentListResponse(docs),
            };

            return serviceDocumentPayload("service_document_listed", data, DbTransactions);
        } catch (error) {
    ...
    private buildQuery(where: any, conditions: any, offset: number): any {
        ...
        if (conditions.fields) {
            query.select(
                (conditions.fields as string).split(",").map((field: string) => field.trim()),
            );
        }
...
export default new listServiceDocumentsService();
