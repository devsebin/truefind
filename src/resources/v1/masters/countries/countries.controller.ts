...
import listCountryService from "./services/list-countries.service";
...
class countryController {
    ...
    async Show(req: Request, res: Response): Promise<JsonResponse | void> {
        ...
        const start = new Date().getTime();
        try {
            response = await showCountryService.execute(new mongoose.Types.ObjectId(req.params.id as string));
            return res.status(response.result.code).json(response.result);
        } catch (error: any) {
            const message = (error as Error).message;
            response = {
