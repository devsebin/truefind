...
import { prioritiesPayload, populateFields } from "../priorities.helper";
import { prioritiesListResponse } from "../priorities.response";
...
class listPrioritiesService {
        ...
        try {
            const [priorities, totalCount] = await Promise.all([
                query
                    ...
                    .exec(),
                this.prioritiesRepository.countDocuments(where),
            ]);
            ...
            const data = {
                ...
                from: 1 + offset,
                rows: prioritiesListResponse(priorities),
            };

            return prioritiesPayload("priorities_listed", data, DbTransactions);
        } catch (error) {
    ...
    private buildQuery(where: any, conditions: any, offset: number): any {
        const query = this.prioritiesRepository.find(where);
        const orderBy: any = {};
...
export default new listPrioritiesService();
