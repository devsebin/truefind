...
import activateServiceEntityHelperService from "../helpers/operations/activate-service-entity.helper.service";
...
class enableServiceEntityService {
  public async execute(
    ...
    try {
      ...
      // 1. Activate parent entity itself
      const saved = await activateServiceEntityHelperService.execute(
        existing,
        session,
        userId,
        dbTransactions,
      ...
      // 2. Activate and cascade descendants (subcategories and services)
      await updateDescendantsHelperService.activate(
        existing,
        session,
        userId,
        dbTransactions,
...
export default new enableServiceEntityService();
