...
import { declaimerListResponse } from "../declaimers.response";
import { declaimerErrorsMessages } from "../declaimers.messages";

class listDeclaimersService {
    ...
    try {
      const [declaimers, totalCount] = await Promise.all([
        query
          ...
          .exec(),
        this.declaimerRepository.countDocuments(where),
      ]);
      ...
      const data = {
        ...
        from: 1 + offset,
        rows: declaimerListResponse(declaimers),
      };

      return declaimerPayload("declaimer_list_fetched", data, DbTransactions);
    } catch (error) {
...
export default new listDeclaimersService();
