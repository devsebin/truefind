...
import { unitsPayload, populateFields } from "../units.helper";
import { unitsListResponse } from "../units.response";
...
class listUnitsService {
        ...
        try {
            const [units, totalCount] = await Promise.all([
                query
                    ...
                    .exec(),
                this.unitsRepository.countDocuments(where),
            ]);
            ...
            const data = {
                ...
                from: 1 + offset,
                rows: unitsListResponse(units),
            };

            return unitsPayload("units_listed", data, DbTransactions);
        } catch (error) {
    ...
    private buildQuery(where: any, conditions: any, offset: number): any {
        const query = this.unitsRepository.find(where);
        const orderBy: any = {};
...
export default new listUnitsService();
