...
class updateBundleStatusesHelperService {
  constructor() {}
  ...
  public async execute(
    ...
    try {
      ...
      if (changes.length === 0) {
        const data = bundleStatusesErrorResponse(existing);
        throwError(
          "no_change_detected",
      ...
      rethrowIfKnown(error, "Error while updating bundle status", errorMap);
    }
  }
}

export default new updateBundleStatusesHelperService();
