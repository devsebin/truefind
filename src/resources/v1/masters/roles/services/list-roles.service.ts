...
import { rolesPayload, populateFields } from "../roles.helper";
import { rolesListResponse } from "../roles.response";
...
class listRolesService {
        ...
        try {
            const [roles, totalCount] = await Promise.all([
                query
                    ...
                    .exec(),
                this.rolesRepository.countDocuments(where),
            ]);
            ...
            const data = {
                ...
                from: 1 + offset,
                rows: rolesListResponse(roles),
            };

            return rolesPayload("roles_listed", data, DbTransactions);
        } catch (error) {
    ...
    private buildQuery(where: any, conditions: any, offset: number): any {
        const query = this.rolesRepository.find(where);
        const orderBy: any = {};
