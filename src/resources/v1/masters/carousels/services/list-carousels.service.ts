...
import { carouselPayload, populateFields } from "../carousels.helper";
import { carouselListResponse } from "../carousels.response";
import { carouselsErrorsMessages } from "../carousels.messages";

class ListCarouselsService {
  ...
  async execute(
    ...
    try {
      const [carousels, totalCount] = await Promise.all([
        query
          .sort({
      ...
      const data = {
        ...
        from: 1 + offset,
        rows: carouselListResponse(carousels),
      };

      return carouselPayload("carousels_listed", data, DbTransactions);
    } catch (error) {
...
export default new ListCarouselsService();
