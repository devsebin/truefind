...
import activateCarouselsHelperService from "../helpers/operations/activate-carousels.helper.service";
...
class EnableCarouselsService {
  public async execute(
    ...
    try {
      ...
      await findCarouselsStateHelperService.isAlreadyActive(
        carousel[0],
        carouselsErrorsMessages,
      );

      await activateCarouselsHelperService.execute(
        carousel[0],
        session,
        userId,
        dbTransactions,
      ...
      return carouselPayload("carousels_activate", carousel, dbTransactions);
    } catch (error) {
...
export default new EnableCarouselsService();
