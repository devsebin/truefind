...
import enableSuburbHelperService from "../helpers/operations/enable-suburb.helper.service";
...
class enableSuburbsService {
  public async execute(
    ...
    try {
      ...
      await findSuburbStateHelperService.isAlreadyActive(
        suburb[0],
        suburbErrorsMessages,
      );

      await enableSuburbHelperService.execute(
        suburb[0],
        session,
        userId,
        dbTransactions,
      ...
      return suburbPayload("suburb_activate", suburb[0], dbTransactions);
    } catch (error) {
...
export default new enableSuburbsService();
