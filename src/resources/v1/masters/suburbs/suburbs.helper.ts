...
import {
    ...
    suburbSuccessMessages,
} from "./suburbs.messages";

export function suburbPayload(
    type: keyof typeof suburbSuccessMessages,
    data: any = [],
    DbTransaction: DbTransaction[] = [],
) {
    const { message, status } = suburbSuccessMessages[type];
    return {
...
export function throwError<T = any>(
    message: keyof typeof suburbErrorsMessages,
    data: ApiResponse<T>,
): never {
