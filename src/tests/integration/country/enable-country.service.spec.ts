...
describe("EnableCountryService (Integration)", () => {
  ...
  beforeEach(async () => {
    ...
    region = await RegionModel.create(
      buildRegionPayload({ country_id: country._id, is_active: false, is_deleted: false, status_id: parentDisabledStatus._id })
    );
  ...
  it("should successfully enable an inactive country", async () => {
    ...
    const regionDb = await RegionModel.findOne({ _id: region._id, is_deleted: false });
    expect(regionDb!.is_active).toBe(true);
    expect(regionDb!.status_id.toString()).toBe(defaultStatus._id.toString());
