...
import disableSuburbsService from "@/resources/v1/masters/suburbs/services/disable-suburbs.service";
...
describe("DisableSuburbService (Integration)", () => {
  let testUser: any;
  let testCountry: any;
  let testRegion: any;
  ...
  it("should successfully disable a suburb and pull ID from district", async () => {
    let result: any;
    await requestContext.run({ userId: testUser._id.toString() }, async () => {
      result = await disableSuburbsService.execute(suburb._id, testUser._id);
    });

    expect(result.result.code).toBe(200);

    // Verify active status
    const disabledDb = await SuburbModel.findOne({ _id: suburb._id, is_deleted: false, is_active: false });
