...
import listSuburbsService from "@/resources/v1/masters/suburbs/services/list-suburbs.service";
...
describe("ListSuburbsService (Integration)", () => {
  let testUser: any;
  let testCountry: any;
  let testRegion: any;
  ...
  it("should successfully list suburbs with pagination", async () => {
    const mockReq = {
      query: {
        page: "1",
        limit: "10",
      },
      originalUrl: "/api/v1/masters/suburbs",
      method: "GET",
    } as any;

    let result: any;
    await requestContext.run({ userId: testUser._id.toString() }, async () => {
      result = await listSuburbsService.execute(mockReq, false);
    });

    expect(result.result.code).toBe(200);
    expect(result.result.success).toBe(true);
    expect(result.result.data[0].result.rows.length).toBe(2);
  });
});
