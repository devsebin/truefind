...
import enableSuburbsService from "@/resources/v1/masters/suburbs/services/enable-suburbs.service";
...
describe("EnableSuburbService (Integration)", () => {
  let testUser: any;
  let testCountry: any;
  let testRegion: any;
  ...
  beforeEach(async () => {
    ...
    // Note: Since it was initialized as inactive, we simulate pulling it out of the district's suburb_ids or start with it empty
    testDistrict.suburb_ids = [];
    await testDistrict.save();
  });

  it("should successfully enable a suburb and push ID to district", async () => {
    let result: any;
    await requestContext.run({ userId: testUser._id.toString() }, async () => {
      result = await enableSuburbsService.execute(suburb._id, testUser._id);
    });

    expect(result.result.code).toBe(200);

    // Verify active status
    const enabledDb = await SuburbModel.findById(suburb._id);
    expect(enabledDb!.is_active).toBe(true);

    // Verify pushed to District
    const updatedDistrict = await DistrictModel.findById(testDistrict._id);
    expect(updatedDistrict!.suburb_ids).toContainEqual(suburb._id);
  });
});
