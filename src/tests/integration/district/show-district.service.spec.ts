...
import showDistrictsService from "@/resources/v1/masters/districts/services/show-districts.service";
...
describe("ShowDistrictService (Integration)", () => {
  let testUser: any;
  let testCountry: any;
  let testRegion: any;
  let district: any;
  ...
  it("should show district details", async () => {
    let showResult: any;
    await requestContext.run({ userId: testUser._id.toString() }, async () => {
      showResult = await showDistrictsService.execute(district._id);
    });
    expect(showResult.result.code).toBe(200);
    expect(showResult.result.data[0].result.name).toBe("Dokki");
  });
});
