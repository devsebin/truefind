...
import listDistrictsService from "@/resources/v1/masters/districts/services/list-districts.service";
...
describe("ListDistrictsService (Integration)", () => {
  let testUser: any;
  let testCountry: any;
  let testRegion: any;
  ...
  beforeEach(async () => {
    ...
    await DistrictModel.create(
      buildDistrictPayload({ name: "Dokki", code: "DKI", country_id: testCountry._id, region_id: testRegion._id })
    );
  });

  it("should list active districts", async () => {
    const mockReq = {
      query: { name: "Dokki" },
      originalUrl: "/api/v1/masters/districts",
      method: "GET",
    } as any;
    let listResult: any;
    await requestContext.run({ userId: testUser._id.toString() }, async () => {
      listResult = await listDistrictsService.execute(mockReq);
    });
    expect(listResult.result.code).toBe(200);
    expect(listResult.result.data[0].result.rows).toBeDefined();
  });
});
